...
gulp.task('oss', function() {
    var data = fs.readFileSync(__dirname+'/build_zip/'+appId+'.zip');
    co(function* () {
      var result = yield client.put(appId+'object-zip', data);
      console.log(result);
    }).catch(function (err) {
      console.log(err);
    });
...
gulp.task("webpack:pre", ['clean'], function(callback) {
    buildTime()
    var myConfig = Object.create(webpackConfig);
...
gulp.task("pre", gulpSequence("webpack:pre","zip"));
